...
function createRig(canvas: HTMLCanvasElement, vw: number, vh: number): Rig {
  ...
  const photos = ["pizza", "pasta", "plates", "coffee", "sips"].map((n) => makePhotoDish(`/dishes/${n}.webp`));
  const group = new THREE.Group();
  photos.forEach((p) => group.add(p.object));
  scene.add(group);
  // One slot per SHAPES item. Round plates turn like a turntable; the shake glass only sways.
  ...
  const floating: FloatingBean[] = Array.from({ length: count }, () => ({
    ...
    ny: Math.random(),
    z: rand(-5, 1.2),
    size: rand(0.05, 0.13) * (vw < 760 ? 0.7 : 1),
    tone: rand(0.55, 0.9),
    rs: rand(-0.6, 0.6),
    r0: rand(0, 6.28),
  }));
