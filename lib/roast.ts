...
export const SHAPES = ["pizza", "pasta", "plates", "bean", "drink"] as const;
...
const toKeyframe = (look: Look, side: "left" | "right" | "center", s: number): Keyframe => ({
  ...
  rough: look.rough,
  coat: look.coat,
  shape: Math.max(0, SHAPES.indexOf(look.shape ?? "pizza")),
});

/** One keyframe per section, in page order: hero, stages, reviews, visit. */
export function buildKeyframes(): Keyframe[] {
  return [
    ...
    toKeyframe(cafe.reviews.look, cafe.reviews.side, 1),
